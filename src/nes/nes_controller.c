...
void nes_write_controller(NES *nes, uint16_t address, uint8_t value)
{
  ...
  {
    nes_log("ERROR: Invalid write address: 0x%04X\n", address);
  }
}
...
uint8_t nes_read_controller(NES *nes, uint16_t address)
{
  ...
  if (address == 0x4016 || address == 0x4017)
  {
    ...
    nes_log("INFO: Read controller state for player %d: 0x%02X\n", player, result);
  }
  else
  {
    nes_log("ERROR: Invalid read address: 0x%04X\n", address);
  }
